...
<html>
  ...
  <body>
	...
	<form method=POST action="/flight/">
	  ...
	  <input type=submit value="send"> updated every 5 min.  <a href="/flight/military/">military aircrafts with ADS-B</a>
	</form>
      </div>
      <div>
	<table class="table table-striped" style="font-size:large">
	  ...
	  <tbody>
	    ...
	    <tr>
	      ...
	      <td><?php print intval((time() - (strtotime($v['created']))) / 60)?> min.</td>
	    </tr>
	      <?php } ?>
	      <?php // } ?>
	  </tbody>
...
<h2>Other Flights</h2>
	<table class="table table-striped" style="font-size:large">
	  ...
	  <tbody>
	    <?php foreach($misc as $k => $v) { ?>
	    <?php // print_r($v)?>
	    <tr>
	      <td><?php print $v['operator']?></td>
	      <td><a href="https://ja.flightaware.com/live/flight/<?php print $v['callsign']?>" target=_blank><?php print $v['callsign']?></a></td>
	      <td><?php print $v['aircraft']?></td>
	      <td><a href="https://flyteam.jp/registration/<?php print $v['registration']?>" target="_blank"><?php print $v['registration']?></a></td>
	    </tr>
...
<?php } ?>
	...
	<div class="container">
	  <ul>
	    ...
	    <li>Powered by: <a href="https://www.adsbexchange.com/" target=_blank>adsbexchange.com</a></li>
	    <li>API to RJAA is <a href="http://tanstaafl.tk/flight/api.json">here.</a></li>
